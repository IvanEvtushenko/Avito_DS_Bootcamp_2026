...
**5. Рефит LR на holdout (mean-of-5)** - test MAP@10 **0.663090**; `experiments/generated/answer_lr_holdout_refit.csv`
...
Запросы короткие (медиана ~10 слов), разговорные, с опечатками и маркерами дат, сумм и тд.. 

Статьи разбираются BeautifulSoup (lxml): удаляются только теги (`script/style/input/img/svg/button`), таблицы, спойлеры/вкладки (`<label>`) (статья на 901К символов укорачиваются до ~506К). 
...
- 100 запросов валидационный выборки были отложены, чтобы уметь +- честно считать метрику локально.
- 400 запросов разбиты на 5 фолдовы, запросы вне каждого фолда (OOF) используются, как валидационная выборка для всех обучаемых стадий (веса fusion, fine-tune веса реранкера): выборка разбир `f` учатся на `train(f)` и оцениваются на `val(f)`.
...
│   ├── rerank/             #   cross-encoder, сменные backend-ы, обучение, майнинг негативов
...
├── docs/                   # детальные отчёты по фазам (см. ниже)
...
- я рад, что Вы дочитали до этого момента!
- очень крутые графики, это классная теория и частично ради того, чтобы видеть вот такое и разбираться с этим, я хочу на стажировку в Авито!

TODO (заключения с ноутбуков с графиками)
- допишу, тут интересно

_

- Была попытка разметить редкие статьи через LLM, задав вопросы к кваждой и уже на этом датасете проводить FT. Идея с треском провалилась, я получил MAP@10 0.226. Проедполагаю, что такое поведение связано с общей проиродой данных. (+ друг по научке занимался гиперболическими эмбеддингами, он сказал, что почти всегда в сетях можно встретить гигантскую компоненту, что я предположительно и увидел)
- В качетсве эмбеддеров я пробовал intfloat/multilingual-e5-large, ai-forever/FRIDA и USER-bge-m3 (дообученный на русской), но остановился на bge-m3
- В качестве реранкеров я пробовал использовать qwen3 0.6B (MAP@10 0.54 без дообучения) (хорошо, вовремя заметил запись на степике про 1B модели и не стал гонять qwen3 4B и 8B квантованную)